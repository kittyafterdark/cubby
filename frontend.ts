...
function cloneConfig(value: CubbyConfig): CubbyConfig {
  return {
    version: 1,
...
function normalizeConfig(input: unknown): CubbyConfig {
  if (!input || typeof input !== 'object') return { version: 1, groups: [] }
  const raw = input as { groups?: unknown }
  if (!Array.isArray(raw.groups)) return { version: 1, groups: [] }

  const seenGroupIds = new Set<string>()
  const seenMemberIds = new Set<string>()
  const groups: CubbyGroup[] = []

  for (const entry of raw.groups) {
    if (!entry || typeof entry !== 'object') continue
    const candidate = entry as Partial<CubbyGroup>
    ...
    if (Array.isArray(candidate.members)) {
      for (const rawMember of candidate.members) {
        ...
        const member = rawMember as Partial<MemberSnapshot>
        if (typeof member.id !== 'string' || !member.id.trim()) continue
        const memberId = member.id.trim()
        if (seenMemberIds.has(memberId)) continue
...
function originLabel(surface: Pick<SpindleHostSurfaceInfo, 'owner'>): string {
  ...
  try {
    ...
    root.querySelectorAll('*').forEach((node) => {
      for (const attr of [...node.attributes]) {
        ...
        if ((name === 'href' || name === 'xlink:href') && /^(?:https?:|javascript:)/.test(value)) node.removeAttribute(attr.name)
      }
    })
...
function fallbackIconSvg(id: string, label: string): string | null {
...
export function setup(ctx: SpindleFrontendContext) {
  ...
  let config: CubbyConfig = { version: 1, groups: [] }
  let surfaces: SpindleHostSurfaceInfo[] = []
  let surfacesById = new Map<string, SpindleHostSurfaceInfo>()
  let manager: ManagerHandle | null = null
  let managerConsumesDrawerSlot = false
  let removeHideStyle: (() => void) | null = null
  let headerMount: Element | null = null
  ...
  function availableSurface(id: string): SpindleHostSurfaceInfo | undefined {
    return surfacesById.get(id)
  }

  function maxGroups(): number {
  ...
  function refreshHideStyle() {
    ...
    for (const group of config.groups) {
      for (const member of group.members) {
        if (availableSurface(member.id)) ids.add(member.id)
      }
    }
    if (!ids.size) return
  ...
  function appendSurfaceIcon(
    box: HTMLElement,
    surface: SpindleHostSurfaceInfo | undefined,
  ...
  function makeSurfaceIcon(
    surface: SpindleHostSurfaceInfo | undefined,
    snapshot: MemberSnapshot | undefined,
    label: string,
    id: string,
    className = 'cubby-tile-icon',
  ): HTMLElement {
  ...
  function renderGroup(group: CubbyGroup, root: HTMLElement) {
    ...
    if (!group.members.length) {
      ...
      for (const member of group.members) {
        ...
        if (live) tile.addEventListener('click', () => void openMember(member.id))
        grid.append(tile)
      }
      stack.append(grid)
    }
  ...
  function assignments(exceptGroupId?: string): Map<string, CubbyGroup> {
    const map = new Map<string, CubbyGroup>()
    for (const group of config.groups) {
  ...
  async function deleteGroup(group: CubbyGroup) {
  ...
  function renderManager() {
    ...
    if (!config.groups.length) {
      ...
      config.groups.forEach((group, index) => {
        ...
        if (folderSvg) cardIcon.append(folderSvg)
  ...
  function updateHeaderBack(activeTabId: string | null) {
  ...
  function handleSurfaceList(next: SpindleHostSurfaceInfo[]) {
    const ownId = ctx.manifest.identifier
    const filtered = next
      .filter((surface) => {
        if (surface.kind !== 'drawer_tab') return false
        if (surface.owner === ownId) return false
        ...
        if (surface.id === 'cubby_manager' || surface.id === 'cubby_compatibility') return false
        if (surface.id.startsWith('cubby_g_')) return false
        return true
      })
      .map((surface) => ({ ...surface }))
    ...
    if (signature === lastSurfaceSignature) return
  ...
  try {
    ...
    const unsubDrawer = ctx.ui.events.onDrawerChange((state: { open: boolean; tabId: string | null }) => {
      if (!headerBackButton && state.open) installHeaderBack()
    ...
    cleanups.push(() => {
      removeHideStyle?.()
      removeHideStyle = null
      for (const runtime of groupRuntimes.values()) runtime.handle.destroy()
      groupRuntimes.clear()
    })
